...
\begin{document}
	...
	\begin{abstract}
	In a previous project we implemented a library using Property Directed Reachability (PDR) for software verification in the program analysis framework ULTIMATE. However there were two problems left: 
	lack of functionality on programs with more than one procedure and the missing usage of interpolants. 
	The goal of this project was to develop techniques to fix both of them. \\
	We devised a paradigm of using our library in a recursive manner to tackle multiple procedure calls and introduce a way of integrating interpolants into the functionality of PDR.
	\end{abstract}
...
\bigskip
	...
	\begin{mydef}
		...
		\end{itemize}
	\end{mydef}
	Interpolant can cover a greater state space than the individual formula A can.
	For example, assume we are given the formulas $A: (x = 5)$ and $B: (x < 0)$, we can see that $A \land B: (x = 5 \land x < 0)$ is unsatisfiable. A possible interpolant $I$ for this pair would be $I: (x > 0)$
	tells us more than just $x = 5$. \\
	Usage of interpolants can speed up the computation of fixpoints in PDR.
	\pagebreak
...
\vspace*{3em} \\
	...
	We see there is only one transition leading to $\ell_E$, $(\ell_1, \ell_E)$. To get the initial proof-obligation, we take transformula $st(\ell_1, \ell_E) = \psi_E$ and use its $\phi$ to create a predicate by substituting every termvariable found in the formula by its corresponding program variable. We get the initial proof-obligation $(x \neq y, \ell_1, 1)$. \\ \\ \par
	...
	\path (4)[densely dotted] edge node {%
		$
		\begin{array}{lcl} \SetToWidest{\psi_{2}} & := & \left\{
		\begin{array}{lcl}
		\invars       & = & \emptyset \\
		\outvars      & = & \emptyset \\
		\auxvars      & = & \emptyset \\
...
\pagestyle{plain}
	...
	To block the proof-obligation $(y \neq x + 1, main_2, 1)$ we need to check each predecessor of $main_2$. There is only one: $inc_1$. $st(inc_1, main_2)$ is the return transition of procedure \texttt{inc}, which necessitates the creation of a new PDR instance. \\ \\
	...
	\end{center}
	The initial proof-obligation of this second instance is $(y \neq x + 1, inc_1, 1)$, the same predicate as in the initial obligation but on $inc_1$. \\
	...
	To block proof-obligation $(y \neq x + 1, inc_1, 1)$ we check the predecessor $inc_0$ of $inc_1$: \\
		\begin{equation*}
		\underbrace{true}_{F_{0, inc_0}}\; \land \; \underbrace{y_{3}' = y_{old_2} + 1}_{st(inc_0, inc_1)\; = \; \psi_1} \land \; \underbrace{y_3' \neq x + 1}_{p'}
		\end{equation*}
	...
	We know now, that it is possible to reach the error through \texttt{inc}. To learn what has to hold for that to happen, we compute the precondition 
	\begin{equation*}
		pre(y \neq x + 1, y_{3}' = y_{old_2} + 1) = (y_{old} \neq x)
	\end{equation*}
	 We have to prove whether $y_{old} \neq x$ holds when entering \texttt{inc} or not. We get a new proof-obligation $(y_{old} \neq x, main_1, 1)$. A third PDR instance is needed. \\ \\
	...
	\vspace*{2em}
	\jw{How does $y_{old} translate to y_0?$}
...
There are no PDR instances left and the program has not been proven unsafe, therefore we can say that $P_2$ is safe.
...
Now, an interpolant for the unsatisfiable formula (3.2) would be $x' = 2$, if we add that to $\ell_1$'s frames, we would be able to block formula (3.1) much earlier.
	\textsc{Ultimate} already supports ways of computing an interpolant for two given formulas. The idea is, that everytime a query to the SMT-solver is unsatisfiable we, instead of adding the negated proof-obligation to the frames, add a calculated interpolant for the query. \par
	...
	To get an interpolant $I$ for that query we define formulae $A$ and $B$, we do that by dividing the query the following way:
	\begin{equation*}
	\underbrace{F_{i - 1, \ell_{pre}} \land T_{\ell_{pre} \rightarrow \ell}}_{A} \land \underbrace{t'}_B
	\end{equation*}
	Using we can bock loops much more efficiently. 
