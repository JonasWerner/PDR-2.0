...
\bigskip
	...
	\begin{mydef} Assume we are given an ICFG $IG = (Loc, \Delta, \ell_{init}, \ell_E, st, proc)$, we find three distinct classes of program transitions in $\Delta$:
		\begin{itemize}
			...
			$st(\ell, \ell')$ consists of two transformulas, $\psi_{ass}$: 
			\begin{itemize}
	...
	Assume we are given program $P_2$ that is represented by ICFG $IG_0 = (Loc_2, \Delta_2, \ell_{init}, \ell_E, st_2, proc_2)$
	\begin{itemize}
		\item  $Loc_2= \{ \ell_0, \ell_1,  \ell_2, \ell_3 \} $
		\item $\Delta_2 = \{ (\ell_0, \ell_1), (\ell_1, \ell_2), (\ell_2,\ell_3), (\ell_3, \ell_4)\}$
		\item $\ell_{init} = \ell_0$ 
		\item $\ell_E = \emptyset$
		\item $st_2 = ((\ell_0, \ell_1) \mapsto \psi_0, (\ell_1, \ell_2) \mapsto \psi_{call}, (\ell_2, \ell_3) \mapsto \psi_1, (\ell_3, \ell_4) \mapsto \psi_{return})$
	...
	\begin{minipage}{.3\textwidth}
		...
		&\texttt{\hspace*{1em} x := 0}; \\
		&\texttt{\hspace*{1em} y := 0;} \\
		&\texttt{\hspace*{1em} call inc();} \\ \\
		&\texttt{procedure inc():} \\
		&\texttt{ \hspace*{1em} y:= y\ +\ 1;} \\
		\end{align*}
	...
	\begin{minipage}{.6\textwidth}
		...
		\node[state] (3) [below=15em of 2] {$\ell_{2}$};
		
		\node[state] (4) [below of=3] {$\ell_{3}$};
		
		\node[state] (5) [below of=4] {$\ell_{4}$};
	...
	\pagebreak
	\jw{Explain what old, non-old etc variables are. Maybe example with assignment of return}		
	...
	In each new iteration there is a new frame added to the program locations. This new frame is initialized as $\{true\}$. An iteration of PDR is divided into two phases, the blocking-phase, and the propagation-phase.
...
\begin{minipage}{.6\textwidth}
		...
		\path (1) edge node {%
			...
			\begin{array}{lcl}
			\invars       & = & \emptyset \\
			\outvars      & = & \{ y \mapsto y_0, \ x \mapsto x_0 \} \\
			\auxvars      & = & \emptyset \\
		...
		(2) edge [loop right] node { $
			\psi_{1} := \left\{
			\begin{array}{lcl}
			\invars       & = & \{ y \mapsto x_0, y \mapsto y_0 \} \\
			\outvars      & = & \{ y \mapsto y_1, \ x \mapsto x_1 \} \\
			\auxvars      & = & \emptyset \\
	...
	\label{ex1} 
\end{minipage}%
\jw{Do I need the other location of the assert? (Not needed for proof and takes up space)}
\vspace*{3em}
