...
\begin{document}
	...
	\begin{figure}[h]
		...
		\path (1) edge node {$TF_0:$ \\ $x := 0;$ \\ $y := 0$} (2)
		;
    ...
    \psi := \left\{
    \begin{array}{lcl}
		\invars       & = & \{x \mapsto x_{0}, y \mapsto y_{0} \} \\
		\outvars      & = & \{ x \mapsto x_{1} , y \mapsto y_{1} \} \\
        \auxvars      & = & \emptyset \\
		\varphi       & = & x_{1} = x_{0} + 1 \; \land \; y_{1} = x_{1} + y_{0} \\
		\hat{\varphi} & = & c_{x}' = c_x + 1 \;\land\; c_{y}' = c_{x}' + c_y \\
    \end{array}
    \right.
    \]%
        } (2)
	...
	If we take both previous example and concatenate them to one CFG, we get \\ $CFG_{1+2} = (\{ \ell_0, \ell_1, \ell_2\}, \{ (\ell_0, \ell_1), (\ell_1, \ell_2)\}, \ell_0, \emptyset, ((\ell_0, \ell_1) \mapsto TF_0, (\ell_1, \ell_2) \mapsto TF_1))$.
		...
		\path (1) edge node {$TF_0:$ \\ $x := 0;$ \\ $y := 0$} (2);
		\path (2) edge node {$TF_1:$ \\ $x := x + 1;$ \\ $y := x + y$} (3);
...
\jw{explain the formulas}
	...
	\jw{Here comes the renaming stuff for Call and Return}
