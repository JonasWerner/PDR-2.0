...
\bigskip
	...
	Assume we are given program $P_2$ that is represented by ICFG $IG_0$ \\ $IG_0 = (\{ \ell_0, \ell_1,  \ell_2, \ell_3 \}, \{ (\ell_0, \ell_1), (\ell_1, \ell_2), \\ (\ell_2,\ell_3), (\ell_3, \ell_4)\}, \ell_0, \emptyset, ((\ell_0, \ell_1) \mapsto \psi_0, (\ell_1, \ell_2) \mapsto \psi_{call}, (\ell_2, \ell_3) \mapsto \psi_1, (\ell_3, \ell_4) \mapsto \psi_{return}))$
	...
	\begin{minipage}{.6\textwidth}
		...
		\path (2)[densely dotted] edge node {%
			...
			\psi_{old} := \left\{
			\begin{array}{lcl}
			\invars       & = & \{y \mapsto y_{in} \} \\
			\outvars      & = & \{ y \mapsto y_{in}, \ y_{old_0} \mapsto y_{old_1} \} \\
			\auxvars      & = & \emptyset \\
			\varphi       & = & y_{old_1} = y_{in} \\
		...
		\path (3) edge node {%
			...
			\right.
			$ %
		} ($(3)!0.3!(4)$) edge[thick] ($(3)!0.6!(4)$) edge
        node[left] {ahahahah}(4)
		;
		...
		\end{tikzpicture}
		\captionof{figure}{$IG_0$}
		\label{fig:rect}
	\end{minipage}%
			\jw{missing $\psi_{global}$; how do i do two arrows in tikz?}
	\pagebreak
