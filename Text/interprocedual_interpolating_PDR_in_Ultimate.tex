...
\pagebreak
	...
	To block the proof-obligation $(y \neq x + 1, main_2, 1)$ we need to check each predecessor of $main_2$. There is only one, $inc_1$, but $st(inc_1, main_2)$ is the return transition of procedure \texttt{inc}, which necessitates the usage of a new PDR instance. \\
	...
	\end{center}
	The initial proof-obligation of this sub-execution is $(y \neq x + 1, inc_1, 1)$, so the same predicate as in the initial obligation, but on $inc_1$. \\
	
	\textsl{5. Step: Sub-PDR Local Frame and First Level Initialization} \\
	For this Sub-PDR we do not need the local frames of \texttt{main} but a new array that integrates only locations found in \texttt{inc}. \\
