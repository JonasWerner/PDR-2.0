...
\begin{document}
	...
	\begin{mydef}
		The closed formula $\hat{\varphi}$ of a formula $\varphi$ is a formula where every variable has been replaced by a constant. Every variable has from the beginning a default constant, this however can be replaced after an assignment of that variable for which we need to construct a new constant for the new equality (This is that with constructConstantOldVarEquality?)
	\end{mydef}
	In our query we use the closed formula of the frame $\hat{F}_{i-1, \ell_{pre}}$, the closed formula of the transfomula $\hat{T}_{\ell_{pre} \rightarrow \ell}$, and the closed formula $\hat{t}'$. To get $\hat{t}'$ we need to construct the primed closed formula that means next to replacing the variables with their constants we need to prime them as well. \\ 
	So in reality we check 	\begin{equation*}
	\hat{F}_{i - 1, \ell_{pre}} \land \hat{T}_{\ell_{pre} \rightarrow \ell} \land \hat{t}'
	\end{equation*}
	for satisfiability. \bigskip
	
	This simple renaming only works on internal transitions however, for call and returns we need to have a look at global variables, variables assigned in the procedure for generating the new constant, and other limitations.
	\jw{todo}
